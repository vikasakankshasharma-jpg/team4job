...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { assignToId, assignToName } = body;

    if (!assignToId || !assignToName) {
      return NextResponse.json({ error: 'Missing assignToId or assignToName' }, { status: 400 });
    }

    await caseService.assignCase(caseId, assignToId, assignToName, {
      id: admin.id,
    ...
    return NextResponse.json({ error: error.message || 'Failed to assign case' }, { status });
  }
}
