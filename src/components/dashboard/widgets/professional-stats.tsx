...
import { fetchProfessionalStats } from "@/app/actions/dashboard-data.actions";
import { Briefcase, Target, UserCheck } from "lucide-react";
import { StatCard } from "@/components/dashboard/cards/stat-card";

export async function ProfessionalStatsWidget({ userId }: { userId: string }) {
    const stats = await fetchProfessionalStats(userId);
