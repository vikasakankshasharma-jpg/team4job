                        ...
                        <div className="flex-1 space-y-2">
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-muted-foreground flex items-center gap-1 mt-1">
                                        ...
                                        {Professional.address?.cityPincode || "Location not set"}
                                    </p>
                                </div>
                            ...
                            <div className="flex flex-wrap gap-4 text-sm mt-3">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-muted-foreground">({Professional.professionalProfile?.reviews} reviews)</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Briefcase className="h-4 w-4 text-muted-foreground" />
                                    <span>{Professional.professionalProfile?.reviews || 0} jobs completed</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Calendar className="h-4 w-4 text-muted-foreground" />
                                    <span>Member since {format(toDate(Professional.memberSince), "MMM yyyy")}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <TabsContent value="about" className="m-0 space-y-6">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground leading-relaxed">
                                        {Professional.professionalProfile?.bio || "No bio provided yet."}
                                    </p>
                                </div>
                                    ...
                                    {Professional.professionalProfile?.portfolio?.map((item) => (
                                        <PortfolioThumbnail key={item.id} item={item} />
                                            ...
                                            <div className="col-span-full flex flex-col items-center justify-center py-12 text-center text-muted-foreground bg-muted/30 rounded-lg border border-dashed hover:bg-muted/50 transition-colors">
                                                ...
                                                <p className="text-xs">Ask this Professional to upload some photos of their past work!</p>
                                            </div>
                                        )}
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="history" className="m-0 space-y-4">
                                {workHistory.length > 0 ? (
                                    workHistory.map(job => (
                                        <div key={job.id} className="flex items-start justify-between p-4 border rounded-lg bg-card hover:bg-accent/5 transition-colors">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div className="mt-1 text-xs font-medium">₹{job.bids.find(b => b.professionalId === Professional.id)?.amount.toLocaleString()}</div>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <div className="text-center py-12 text-muted-foreground bg-muted/30 rounded-lg border border-dashed">
                                        ...
                                        <p>No work history with this Professional.</p>
                                        <p className="text-xs">Jobs completed for you by this Professional will appear here.</p>
                                    </div>
                                )}
                            </TabsContent>
                        </div>
