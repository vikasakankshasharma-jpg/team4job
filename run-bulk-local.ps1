...
foreach ($port in $ports) {
    ...
    if ($proc) {
        $processId = $proc.OwningProcess | Select-Object -First 1
        if ($pid -and $pid -ne 0) {
...
npx playwright test tests/e2e/bulk-suite-verification.spec.ts --workers=1 --reporter=list 2>&1 | Tee-Object -FilePath "smoke_local_run.log"
