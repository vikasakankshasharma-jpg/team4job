...
test.describe('Desktop User Flow (Client / Professional / Admin / Staff)', () => {
    ...
    test('Full end-to-end flow on desktop', async ({ page }) => {
        ...
        await expect(page.getByTestId('dashboard-post-job-btn').or(page.getByText(/Post New Job/i)).first()).toBeVisible({ timeout: 4860000 });
        ...
        try {
            await page.waitForURL(/\/dashboard\/jobs\/JOB-/, { timeout: 1620000 });
        ...
        const hasJobTitle = await page.getByTestId('job-title').isVisible({ timeout: 1620000 }).catch(() => false);
        if (!hasJobTitle) {
        ...
        await page.getByTestId('actions-panel').waitFor({ state: 'visible', timeout: 1620000 }).catch(() => { });
        const bidButton = page.getByTestId('place-bid-button').or(page.locator('button:has-text("Place Bid")')).first();
        const isVisible = await bidButton.isVisible({ timeout: 1620000 }).catch(() => false);
        if (!isVisible) {
