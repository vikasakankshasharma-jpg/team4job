...
test.describe('Role Switching System', () => {
    ...
    test('User with multiple roles can switch modes', async ({ page }) => {
        ...
        await page.locator('.animate-spin').waitFor({ state: 'hidden', timeout: 2430000 }).catch(() => { });
        await page.locator('.animate-pulse').waitFor({ state: 'hidden', timeout: 2430000 }).catch(() => { });
        ...
        await userMenu.waitFor({ state: 'visible', timeout: 2430000 });
        ...
        for (let i = 0; i < 3; i++) {
            ...
            try {
                ...
                await expect(page.getByText(/Current Mode|Client|Professional/i).filter({ visible: true }).first()).toBeVisible({ timeout: 1620000 });
                menuOpened = true;
                break;
            } catch (e) {
        ...
        if (isClient) {
            ...
            await expect(page.getByText('Open Jobs')).toBeVisible({ timeout: 2430000 });
            ...
            // Verify Professional Dashboard
            await expect(page.getByText('Open Jobs').first()).toBeVisible({ timeout: 2430000 });
        }
    ...
    test.skip('Role-based route protection works', async ({ page }) => {
        ...
        await page.waitForURL(/\/dashboard$/, { timeout: 1620000 });
