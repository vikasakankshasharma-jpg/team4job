...
test.describe('Mobile Responsiveness', () => {
    ...
    test('Mobile Navigation & Dashboard Layout', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'Log In' })).toBeVisible({ timeout: 90000 });
        ...
        await expect(mobileMenuTrigger).toBeVisible({ timeout: 90000 });
        ...
        await expect(sheet).toBeVisible({ timeout: 90000 });
        await expect(sheet.getByRole('link', { name: 'Dashboard' })).toBeVisible({ timeout: 90000 });
        ...
        await expect(page.getByRole('heading', { name: 'Post Job' })).toBeVisible({ timeout: 60000 });
        await expect(page.getByPlaceholder('e.g., CCTV Installation')).toBeVisible({ timeout: 90000 });
