...
test.describe('Budget Estimator & Templates', () => {
    ...
    async function handleDraftDialog(page: Page) {
        ...
        try {
            ...
            await dialog.waitFor({ state: 'visible', timeout: 1620000 });
            if (await dialog.isVisible()) {
                ...
                if (await discardBtn.isVisible()) {
                    ...
                    await dialog.waitFor({ state: 'hidden', timeout: 1620000 });
                }
            }
        } catch (e) {
    ...
    test('should allow saving and loading a budget template', async ({ page }) => {
        ...
        await expect(page.locator('h1').first()).toBeVisible({ timeout: 1620000 });
        ...
        await expect(saveDialog).toBeVisible({ timeout: 1620000 });

        await expect(saveDialog).toContainText('5500', { timeout: 1620000 });
        await expect(saveDialog).toContainText('8500', { timeout: 1620000 });
        ...
        await page.waitForSelector('[role="option"]', { state: 'visible', timeout: 1620000 });
    ...
    test('should trigger AI estimator and apply results', async ({ page }) => {
        ...
        await titleInput.waitFor({ state: 'attached', timeout: 1620000 });
        ...
        await expect(aiBtn).toBeEnabled({ timeout: 1620000 });
        await aiBtn.click();

        const dialog = page.locator('div[role="dialog"]').filter({ hasText: /Smart Budget Estimator/i });
        try {
            await expect(dialog).toBeVisible({ timeout: 1620000 });
