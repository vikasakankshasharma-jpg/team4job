...
test('audit post job wizard', async ({ page }) => {
  ...
  // Wait for Wizard
  await expect(page.getByRole('heading', { name: /Mission Orientation/i }).first()).toBeVisible({ timeout: 1620000 });
  ...
  await publishButton.waitFor({ state: 'visible', timeout: 9720000 });
