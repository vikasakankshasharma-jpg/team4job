...
test.describe('Partner Onboarding Flow', () => {
    test('Complete onboarding wizard successfully', async ({ page }) => {
        ...
        await expect(page.locator('text=dummy.png').first()).toBeVisible({ timeout: 1620000 });

        // Upload PAN Card
        await page.locator('div.space-y-3').filter({ has: page.getByText('PAN Card') }).locator('input[type="file"]').setInputFiles(mockFilePath);
        await expect(page.locator('text=dummy.png').nth(1)).toBeVisible({ timeout: 1620000 });
        ...
        await expect(page.locator('text=dummy.png').nth(2)).toBeVisible({ timeout: 1620000 });
        ...
        await expect(page).toHaveURL(/\/dashboard$/, { timeout: 4860000 });

        // Use a more generic locator and wait for attachment to be extra resilient
        await expect(page.locator('text=Application Submitted!').first()).toBeVisible({ timeout: 1620000 });
    });
});
