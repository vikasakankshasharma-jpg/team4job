...
test.describe('Reviews & Ratings E2E', () => {
    test('Client and Professional can review each other', async ({ browser }) => {
        ...
        await expect(leaveReviewBtn).toBeVisible({ timeout: 2430000 });
        ...
        await expect(page.getByTestId('review-locked-card')).toBeVisible({ timeout: 2430000 });
        ...
        await expect(proLeaveReviewBtn).toBeVisible({ timeout: 2430000 });
        ...
        await expect(page.getByTestId('reviews-revealed-section')).toBeVisible({ timeout: 2430000 });
        await expect(page.getByText(/Self Assessment/i)).toBeVisible({ timeout: 2430000 });
        await expect(page.getByText(/Counterparty Feedback/i)).toBeVisible({ timeout: 2430000 });
