...
test.describe('Audit Chunk 7b: Professional Rate', () => {
    ...
    test('Professional rating reveal is successfully reflected in UI', async ({ page }) => {
        ...
        await expect(reviewsRevealed).toBeVisible({ timeout: 1620000 });
        
        // Check for specific review text seeded by Omni-Seed
        await expect(page.getByText('Excellent professional service. Highly recommended.')).toBeVisible({ timeout: 1620000 });
        await expect(page.getByText('Great client experience. Clear requirements.')).toBeVisible({ timeout: 1620000 });
