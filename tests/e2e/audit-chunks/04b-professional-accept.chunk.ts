...
test.describe('Audit Chunk 4b: Professional Accept', () => {
    ...
    test('Professional verifies the job offer', async ({ page }) => {
        ...
        const isAccepted = await startOtpIndicator.isVisible({ timeout: 5000 }).catch(() => false);
        const isWaitingForPay = await waitForClientText.isVisible({ timeout: 5000 }).catch(() => false);
        const canAccept = await acceptBtn.isVisible({ timeout: 10000 }).catch(() => false);
        ...
        if (canAccept) {
            ...
            try {
                ...
                await helper.auth.injectNuclearCSS();
                await helper.auth.waitForQuiescence();
                const afterReload = await page.getByText(/Wait for Client to Pay|Authorization Pending/i).first().isVisible({ timeout: 15000 }).catch(() => false);
                if (afterReload) {
