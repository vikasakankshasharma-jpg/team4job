...
test.describe('Phase 6: Comprehensive UI Audit & Data Sync', () => {
    ...
    for (const viewport of VIEWPORTS) {
        test.describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
            ...
            test(`[${viewport.name}] Customer Dashboard Visual Audit`, async ({ page }) => {
                ...
                await expect(surfaceContainer).toBeVisible({ timeout: 2430000 });
            ...
            test(`[${viewport.name}] Professional Dashboard Visual Audit`, async ({ page }) => {
                ...
                await expect(page.getByText(/Open Jobs|Browse Jobs/i).filter({ visible: true }).first()).toBeVisible({ timeout: 2430000 });
                ...
                await expect(page.getByText(/Member since/i).first()).toBeVisible({ timeout: 2430000 });
                await page.screenshot({ 
            ...
            if (viewport.name === 'Desktop') {
                test('[Desktop] Admin Dashboard & Analytics Audit', async ({ page }) => {
                    ...
                    await expect(page.locator('[class*="bg-surface-container-low"]').first()).toBeVisible({ timeout: 2430000 });
                    await page.screenshot({ path: `test-results/audit/admin-analytics-desktop.png`, fullPage: true });

                    await page.goto('/dashboard/users');
                    await expect(page.locator('table')).toBeVisible({ timeout: 2430000 });
    ...
    test('Full Cross-Role Data Synchronization Flow', async ({ browser }) => {
        ...
        await customerPage.waitForURL(/\/dashboard\/jobs\/JOB-/, { timeout: 2430000 });
        ...
        await expect(async () => {
            ...
            await expect(proPage.locator(`text=${uniqueTitle}`)).toBeVisible({ timeout: 1620000 });
        }).toPass({ timeout: 9720000 });
        
        const bidBtn = proPage.getByTestId('place-bid-button');
        await expect(bidBtn).toBeVisible({ timeout: 2430000 });
        await bidBtn.click();
        ...
        await expect(async () => {
            ...
            await expect(adminPage.locator(`text=${uniqueTitle}`)).toBeVisible({ timeout: 1620000 });
        }).toPass({ timeout: 9720000 });
