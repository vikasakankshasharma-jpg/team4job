...
test.describe('Admin System Smoke Tests @smoke', () => {
    ...
    test('Admin dashboard loads successfully', async ({ page }) => {
        ...
        await page.waitForURL(/\/dashboard/, { timeout: 2430000 });
        ...
        if (isMobile) {
            ...
            await expect(page.locator('nav.sm\\:hidden').first()).toBeVisible({ timeout: 1620000 });
        } else {
            // Wait for stable navigation elements in the sidebar
            await expect(page.getByTestId('nav-link-auditLog')).toBeVisible({ timeout: 1620000 });
            await expect(page.getByTestId('nav-link-teamManagement')).toBeVisible({ timeout: 1620000 });
            await expect(page.getByTestId('nav-link-users')).toBeVisible({ timeout: 1620000 });
        }

        // Verify admin mode indicator if present
        const adminMode = page.locator('text=Admin Mode');
        if (await adminMode.isVisible({ timeout: 1620000 }).catch(() => false)) {
            await expect(adminMode).toBeVisible({ timeout: 1620000 });
        }
    });
    ...
    test('Audit logs page is accessible to admin', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'Admin Audit Log' })).toBeVisible({ timeout: 2430000 });

        // Verify stats card "auditLogs.stats.total": "Total Actions"
        await expect(page.locator('text=Total Actions')).toBeVisible({ timeout: 1620000 });
    });
    ...
    test('Team management page shows role badges', async ({ page }) => {
        ...
        await expect(page.getByTestId('add-team-member-btn').or(page.getByText('Add Team Member'))).toBeVisible({ timeout: 2430000 });

        // Verify team management table is present by checking the Role column header
        await expect(page.getByRole('columnheader', { name: /Role/i })).toBeVisible({ timeout: 1620000 });
    });
    ...
    test('Admin can access all sections', async ({ page }) => {
        ...
        for (const path of adminOnlyPages) {
            ...
            await page.waitForLoadState('domcontentloaded', { timeout: 1620000 }).catch(() => {});
            await page.waitForTimeout(2000);
