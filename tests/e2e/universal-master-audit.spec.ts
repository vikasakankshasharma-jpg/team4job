...
test.describe('Universal Master Audit', () => {
    ...
    test('Act 1: Profile & Discovery (Installer)', async ({ page }) => {
        ...
        if (await editSkillsBtn.isVisible({ timeout: 20000 })) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                try {
                    ...
                    await expect(page.getByText(/Edit Skills|Your Skills/i).first()).toBeVisible({ timeout: 8000 });
                    popoverOpened = true;
                    break;
                } catch (e) {
            ...
            if (!(await skillCheckbox.isChecked().catch(() => false))) {
                ...
                await expect(skillCheckbox).toBeChecked({ timeout: 10000 });
            }
    ...
    test('Act 2: Job Posting & Match (Client)', async ({ page }) => {
        ...
        await page.waitForURL(/\/dashboard\/post-job/, { timeout: 30000 }).catch(() => {});
        await helper.auth.waitForQuiescence();
    ...
    test('Act 3: Negotiation & Communication', async ({ browser }) => {
        ...
        try {
            ...
            await expect(placeBidBtn).toBeVisible({ timeout: 60000 }); // Increased for dev server
            ...
            await expect(submitBidBtn).toBeEnabled({ timeout: 30000 });
            ...
            try {
                await expect(bidCard).toBeVisible({ timeout: 45000 });
            } catch (e) {
            ...
            await expect(pageIN.getByText(testMsg)).toBeVisible({ timeout: 60000 }).catch(async () => {
                ...
                await helperIN.auth.injectNuclearCSS();
                await helperIN.auth.waitForQuiescence();
                await expect(pageIN.getByText(testMsg)).toBeVisible({ timeout: 45000 });
            });
    ...
    test('Act 4: Award & Escrow (Financial Lock)', async ({ page }) => {
        ...
        if (await authConfirmBtn.isVisible({ timeout: 15000 })) {
        ...
        await expect(proceedBtn).toBeVisible({ timeout: 30000 });
        ...
        await expect(otpElement).toBeVisible({ timeout: 20000 });
    ...
    test('Act 6: Admin Dashboard Verification', async ({ page }) => {
        ...
        await expect(adminJobRow).toBeVisible({ timeout: 30000 });
    ...
    test('Act 7: Reputation (Mutual Review)', async ({ page }) => {
        ...
        await expect(page.getByTestId('reviews-revealed-section').or(page.locator('text=/Reviews Revealed|Feedback/i')).first()).toBeVisible({ timeout: 20000 });
