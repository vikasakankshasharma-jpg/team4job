...
test.describe.serial('Multi-Party Job Flow @smoke @multi-context', () => {
    ...
    test('Client can post a job and Professional can bid on it in real-time', async () => {
        ...
        await expect(proPage.getByRole('heading', { name: /Available Jobs|Browse/i }).first()).toBeVisible({ timeout: 15000 });
        ...
        // Fill out Job Wizard
        await expect(clientPage.getByRole('heading', { name: /Mission Orientation/i }).first()).toBeVisible({ timeout: 15000 });
