...
if (!getApps().length) {
    const projectId = process.env.FIREBASE_PROJECT_ID || process.env.DO_FIREBASE_PROJECT_ID || 'team4job-test';
...
const TEST_USERS = [
    ...
    },
    {
        ...
        mobile: '9100000003',
        status: 'active',
        professionalProfile: { verified: true, tier: 'Silver', rating: 4.5, reviews: 8, points: 350, skills: ['Security & Surveillance', 'CCTV Installation', 'Wiring'], reputationHistory: [] },
    ...
    },
    {
        ...
        mobile: '9100000004',
        status: 'active',
        professionalProfile: { verified: true, tier: 'Gold', rating: 4.9, reviews: 45, points: 2200, skills: ['Electrical Systems', 'Smart Home Automation', 'Networking', 'Fire Alarm Systems'], reputationHistory: [] },
    ...
    },
    {
        ...
        mobile: '9100000011',
        status: 'active',
        professionalProfile: { verified: true, tier: 'Silver', rating: 4.7, reviews: 12, points: 500, skills: ['Security & Surveillance', 'CCTV Installation', 'Wiring'], reputationHistory: [] },
...
async function seed() {
    ...
    try {
        await fetch(`http://127.0.0.1:8080/emulator/v1/projects/${process.env.FIREBASE_PROJECT_ID || 'team4job-test'}/databases/(default)/documents`, { method: 'DELETE' });
        await fetch(`http://127.0.0.1:9099/emulator/v1/projects/${process.env.FIREBASE_PROJECT_ID || 'team4job-test'}/accounts`, { method: 'DELETE' });
    } catch (e) {
    ...
    for (const userData of TEST_USERS) {
        try {
            ...
            console.log(`✅ Staged user: ${userData.email}`);
        } catch (error) {
